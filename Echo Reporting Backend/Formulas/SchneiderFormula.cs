...
namespace FetalReporting.Formulas {
    public class SchneiderFormula : IFormula {
        ...
        public double GetZScore(double measurement) {
            ...
            return (Math.Log(measurement / 10) - predCardDim) / constants.MSE;
        }
        public bool ZScoreable() => constants.Pd.PatientAge.Value >= 3;
        public static SchneiderFormula MVDecelTime(PatientData pd, string name) {
            double[] averages = { 145, 157, 172 };
            double[] sds = { 18, 19, 22 };
            return new SchneiderFormula(new Constants(averages, sds, pd, name, new[] { "Shortened", "", "Prolonged" }));
        }
        public static SchneiderFormula LVIVRT(PatientData pd, string name) {
            double[] averages = { 62, 67, 74 };
            double[] sds = { 10, 10, 13 };
            return new SchneiderFormula(new Constants(averages, sds, pd, name, new[] { "Shortened", "Normal", "Prolonged" }));
        }
